...
SarReader::SarReader(const char *path, const unsigned char *key_table) : DirectReader(path, key_table) {
  root_archive_info = last_archive_info = &archive_info;
  num_of_sar_archives = 0;
}

SarReader::~SarReader() { close(); }
...
void SarReader::readArchive(ArchiveInfo *ai, int archive_type, unsigned int offset) {
  ...
  if (archive_type == ARCHIVE_TYPE_NS2) {
    ...
    for (i = 0; i < ai->num_of_files; i++) {
      ...
      if (ai->fi_list[i].compression_type == NBZ_COMPRESSION || ai->fi_list[i].compression_type == SPB_COMPRESSION) {
...
size_t SarReader::putFileSub(ArchiveInfo *ai, FILE *fp, int no, size_t offset, size_t length, size_t original_length,
                             int compression_type, bool modified_flag, unsigned char *buffer) {
  ...
  if (modified_flag) {
    if (ai->fi_list[no].compression_type == NBZ_COMPRESSION) {
      ...
      if (readChar(ai->file_handle) != 'B' ||
          readChar(ai->file_handle) != 'Z') { // in case the original is not compressed in NBZ
...
size_t SarReader::putFile(FILE *fp, int no, size_t offset, size_t length, size_t original_length, bool modified_flag,
                          unsigned char *buffer) {
  ArchiveInfo *ai = archive_info.next;
  return putFileSub(ai, fp, no, offset, length, original_length, ai->fi_list[no].compression_type, modified_flag,
                    buffer);
}
...
const char *SarReader::getArchiveName() const { return "sar"; }
...
int SarReader::getIndexFromFile(ArchiveInfo *ai, const char *file_name) {
  ...
  for (i = 0; i < ai->num_of_files; i++) {
    if (!strcmp(capital_name, ai->fi_list[i].name))
      break;
  }
...
size_t SarReader::getFileSub(ArchiveInfo *ai, const char *file_name, unsigned char *buf) {
  ...
  if (key_table_flag)
    for (size_t j = 0; j < ret; j++)
      buf[j] = key_table[buf[j]];
  return ret;
}

size_t SarReader::getFile(const char *file_name, unsigned char *buf, int *location) {
  size_t ret;
  if ((ret = DirectReader::getFile(file_name, buf, location)))
    return ret;

  ArchiveInfo *info = archive_info.next;
  size_t j = 0;
  for (int i = 0; i < num_of_sar_archives; i++) {
    if ((j = getFileSub(info, file_name, buf)) > 0)
      break;
    info = info->next;
  }
  if (location)
...
SarReader::FileInfo SarReader::getFileByIndex(unsigned int index) {
  ArchiveInfo *info = archive_info.next;
  for (int i = 0; i < num_of_sar_archives; i++) {
    if (index < info->num_of_files)
      return info->fi_list[index];
