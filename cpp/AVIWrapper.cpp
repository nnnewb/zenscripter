/* -*- C++ -*-
 *
 *  AVIWrapper.cpp - avifile library wrapper class to play AVI video & audio
 * stream
 *
...
int AVIWrapper::initAV(SDL_Surface *surface, bool audio_open_flag) {
  ...
  if (debug_flag)
    printf(" format %d ch %d sample %d bit %d avg Bps %d\n", wave_fmt.wFormatTag, wave_fmt.nChannels,
           wave_fmt.nSamplesPerSec, wave_fmt.wBitsPerSample, wave_fmt.nAvgBytesPerSec);
...
int AVIWrapper::playVideo(void *userdata) {
  ...
  while (status == AVI_PLAYING && !v_stream->Eof()) {
    ...
    for (i = 0; i < NUM_CACHES; i++) {
      if (cache[i].valid) {
        if (nearest_cache == -1 || (nearest_cache >= 0 && cache[i].time < cache[nearest_cache].time)) {
          nearest_cache = i;
    ...
    // getAudioTime(), minimum_time, nearest_cache, minimum_time, current_time
    // );
    if (async < -0.01) {
      if (remaining_cache == 0) {
        // printf("sync0 %f %f %f %f\n", async,
        // (a_stream)?a_stream->GetTime():0.0, v_stream->GetTime(), minimum_time
        // );
        SDL_Delay((int)(-async * 1000));
      }
    }
    if ((async < -0.01 && remaining_cache > 0) || nearest_cache >= 0) {
      // add cache
      for (i = 0; i < NUM_CACHES; i++) {
