...
#define IS_TWO_BYTE(x) (((unsigned char)(x) > (unsigned char)0x80) && ((unsigned char)(x) != (unsigned char)0xff))
...
void DirectReader::writeChar(FILE *fp, unsigned char ch) { fwrite(&ch, 1, 1, fp); }
...
unsigned short DirectReader::swapShort(unsigned short ch) { return ((ch & 0xff00) >> 8) | ((ch & 0x00ff) << 8); }
...
int DirectReader::open(const char *name) { return 0; }

int DirectReader::close() { return 0; }

const char *DirectReader::getArchiveName() const { return "direct"; }

int DirectReader::getNumFiles() { return 0; }
