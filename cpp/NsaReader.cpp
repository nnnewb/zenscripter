...
int NsaReader::open(const char *nsa_path) {
  ...
  if (archive_type & ARCHIVE_TYPE_NS2) {
    for (i = 0; i < MAX_NS2_ARCHIVE; i++) {
      ...
      if ((archive_info_ns2[i].file_handle = fopen(archive_name, "rb")) == NULL)
        break;
  ...
  if (num_of_ns2_archives == 0 && archive_type & ARCHIVE_TYPE_NSA) {
    for (i = -1; i < MAX_EXTRA_ARCHIVE; i++) {
      ...
      if ((ai->file_handle = fopen(archive_name, "rb")) == NULL)
        break;
...
size_t NsaReader::putFile(FILE *fp, int no, size_t offset, size_t length, size_t original_length, int compression_type,
...
size_t NsaReader::getFileLength(const char *file_name) {
  ...
  if ((ret = getFileLengthSub(&archive_info, file_name)))
    return ret;
...
NsaReader::FileInfo NsaReader::getFileByIndex(unsigned int index) {
  ...
  if (index < archive_info.num_of_files)
