...
void ONScripter::initSDL() {
  ...
  if (custom_screen_device_height == -1 || custom_screen_device_width == -1) {
    ...
    window = SDL_CreateWindow(NULL, window_x, window_y, custom_screen_device_width, custom_screen_device_height,
                              window_flag);
  }
  if (window == NULL) {
...
void ONScripter::openAudio(int freq) {
  ...
  if (Mix_OpenAudio((freq < 0) ? 44100 : freq, MIX_DEFAULT_FORMAT, MIX_DEFAULT_CHANNELS, DEFAULT_AUDIOBUF) < 0) {
    ...
    utils::printInfo("Audio: %d Hz %d bit %s\n", freq, (format & 0xFF), (channels > 1) ? "stereo" : "mono");
...
void ONScripter::setCDNumber(int cdrom_drive_number) { this->cdrom_drive_number = cdrom_drive_number; }

void ONScripter::setFontFile(const char *filename) { setStr(&default_font, filename); }

void ONScripter::setRegistryFile(const char *filename) { setStr(&registry_file, filename); }

void ONScripter::setDLLFile(const char *filename) { setStr(&dll_file, filename); }
...
void ONScripter::setCompatibilityMode() { compatibilityMode = true; }

void ONScripter::setFontCache() { cacheFont = true; }

void ONScripter::enableButtonShortCut() { force_button_shortcut_flag = true; }

void ONScripter::enableWheelDownAdvance() { enable_wheeldown_advance_flag = true; }
...
void ONScripter::setKeyEXE(const char *filename) { setStr(&key_exe_file, filename); }
...
#if SDL_VERSION_ATLEAST(2, 0, 0)
  texture = SDL_CreateTexture(renderer, texture_format, SDL_TEXTUREACCESS_STREAMING, accumulation_surface->w,
...
void ONScripter::flushDirect(SDL_Rect &rect, int refresh_mode) {
  ...
  SDL_UpdateTexture(texture, &rect,
                    (unsigned char *)accumulation_surface->pixels + accumulation_surface->pitch * rect.y +
...
#ifdef USE_SDL_RENDERER
  SDL_Rect dst_rect = {(device_width - screen_device_width) / 2, (device_height - screen_device_height) / 2,
...
void ONScripter::mouseOverCheck(int x, int y) {
  ...
  while (bl) {
    if (x >= bl->select_rect.x && x < bl->select_rect.x + bl->select_rect.w && y >= bl->select_rect.y &&
      ...
      if (transbtn_flag == false || shift_over_button == bl->no) {
        ...
        if ((bl->button_type == ButtonLink::SPRITE_BUTTON &&
            ...
            (bl->button_type == ButtonLink::NORMAL_BUTTON && max_alpha < (alpha = bl->anim[0]->getAlpha(x, y)))) {
          max_alpha = alpha;
          max_bl = bl;
          max_c = c;
        }
  ...
  if (current_over_button != button) {
    ...
    if (bl) {
      ...
      if (bl->button_type == ButtonLink::SPRITE_BUTTON) {
        ...
        bl->anim[0]->setCell(1);
      } else if (bl->button_type == ButtonLink::NORMAL_BUTTON || bl->button_type == ButtonLink::LOOKBACK_BUTTON) {
        bl->show_flag = 1;
      }
...
void ONScripter::setFullScreen(bool fullscreen) {
  if (fullscreen != fullscreen_mode) {
    ...
    if (!SDL_WM_ToggleFullScreen(screen_surface)) {
      screen_surface =
          SDL_SetVideoMode(screen_device_width, screen_device_height, screen_bpp,
                           fullscreen ? DEFAULT_VIDEO_SURFACE_FLAG | SDL_FULLSCREEN : DEFAULT_VIDEO_SURFACE_FLAG);
      flushDirect(screen_rect, refreshMode());
    }
#else
    fullscreen_mode = fullscreen;
...
void ONScripter::set_accelerated_render_mode(bool s) { accelerated_render_mode = s; }

bool ONScripter::get_accelerated_render_mode() { return accelerated_render_mode; }

void ONScripter::set_custom_screen_device_height(int x) { custom_screen_device_height = x; }

void ONScripter::set_custom_screen_device_width(int x) { custom_screen_device_width = x; }
  ...
  while (current_line < current_label_info.num_of_lines) {
    if (debug_level > 0)
      utils::printInfo("*****  executeLabel %s:%d/%d:%d:%d *****\n", current_label_info.name, current_line,
                       current_label_info.num_of_lines, string_buffer_offset, display_mode);
