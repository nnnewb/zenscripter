...
public:
  ...
  void NSDSp2Command(int texnum, int dcx, int dcy, int sx, int sy, int w, int h, int xs, int ys, int rot, int alpha);
...
private:
  ...
  enum { ALPHA_BLEND_CONST = 1, ALPHA_BLEND_MULTIPLE = 2, ALPHA_BLEND_FADE_MASK = 3, ALPHA_BLEND_CROSSFADE_MASK = 4 };
...
private:
  ...
  // format = SDL_PIXELFORMAT_ABGR8888 for OpenGL ES 1.x, OpenGL ES 2.x
  // (Android, iOS) format = SDL_PIXELFORMAT_ARGB8888 for OpenGL, Direct3D
  // (Windows, Linux, MacOSX) or for any 32bit surface without SDL_Renderer
  // format = SDL_PIXELFORMAT_RGB565 for any 16bit surface without SDL_Renderer
  // (Android, Zaurus)
  Uint32 texture_format;
  SDL_Surface *accumulation_surface; // Final image, i.e. picture_surface (+
                                     // shadow + text_surface)
  SDL_Surface *backup_surface;       // Final image w/o (shadow + text_surface) used
                                     // in leaveTextDisplayMode()
  ...
  void alphaBlendText(SDL_Surface *dst_surface, SDL_Rect dst_rect, SDL_Surface *src_surface, SDL_Color &color,
  ...
  enum { WAVE_PLAY = 0, WAVE_PRELOAD = 1, WAVE_PLAY_LOADED = 2 };
  ...
  enum { DISPLAY_MODE_NORMAL = 0, DISPLAY_MODE_TEXT = 1 };
  int display_mode;
  ...
  void drawGlyph(SDL_Surface *dst_surface, FontInfo *info, SDL_Color &color, char *text, int xy[2],
                 AnimationInfo *cache_info, SDL_Rect *clip, SDL_Rect &dst_rect);
  void drawChar(char *text, FontInfo *info, bool flush_flag, bool lookback_flag, SDL_Surface *surface,
                AnimationInfo *cache_info, SDL_Rect *clip = NULL);
  void drawString(const char *str, uchar3 color, FontInfo *info, bool flush_flag, SDL_Surface *surface,
                  SDL_Rect *rect = NULL, AnimationInfo *cache_info = NULL, bool pack_hankaku = true);
