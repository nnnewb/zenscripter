...
int ONScripter::calcDurationToNextAnimation() {
  ...
  if (!textgosub_label && (clickstr_state == CLICK_WAIT || clickstr_state == CLICK_NEWPAGE)) {
    AnimationInfo *anim;
...
void ONScripter::setupAnimationInfo(AnimationInfo *anim, FontInfo *info) {
  if (anim->trans_mode != AnimationInfo::TRANS_STRING && anim->file_name && anim->surface_name &&
    ...
    if (buffer[0] == 'a') {
      ...
      if (*buffer == '/') {
        ...
        if (script_h.getEndStatus() & ScriptHandler::END_COMMA) {
          ...
          if (script_h.getEndStatus() & ScriptHandler::END_COMMA) {
            script_h.readInt(); // 0 ... normal, 1 ... no anti-aliasing, 2 ...
                                // Fukuro
          }
        }
