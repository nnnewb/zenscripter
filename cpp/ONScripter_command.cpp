...
int ONScripter::yesnoboxCommand() {
  ...
  while (1) {
    ...
    if (current_button_state.button == -1 || current_button_state.button == 2) {
...
int ONScripter::sp_rgb_gradationCommand() {
  ...
  ONSBuf key_mask = (((key_r >> fmt->Rloss) << fmt->Rshift) | ((key_g >> fmt->Gloss) << fmt->Gshift) |
  ...
  for (i = upper_bound; i <= lower_bound; i++) {
    ...
    if (upper_bound != lower_bound) {
      color |= (((lower_r - upper_r) * (i - upper_bound) / (lower_bound - upper_bound) + upper_r) >> fmt->Rloss)
               << fmt->Rshift;
      color |= (((lower_g - upper_g) * (i - upper_bound) / (lower_bound - upper_bound) + upper_g) >> fmt->Gloss)
               << fmt->Gshift;
      color |= (((lower_b - upper_b) * (i - upper_bound) / (lower_bound - upper_bound) + upper_b) >> fmt->Bloss)
               << fmt->Bshift;
    } else {
...
int ONScripter::spbtnCommand() {
  ...
  int sprite_no = script_h.readInt();
  int no = script_h.readInt();
  if (no < 1 || sprite_no < 0 || sprite_no >= MAX_SPRITE_NUM || sprite_info[sprite_no].image_surface == NULL)
    return RET_CONTINUE;
...
int ONScripter::selectCommand() {
  ...
  if (selectvoice_file_name[SELECTVOICE_OPEN])
    playSound(selectvoice_file_name[SELECTVOICE_OPEN], SOUND_CHUNK, false, MIX_WAVE_CHANNEL);
  ...
  while (1) {
    if (script_h.getNext()[0] != 0x0a && comma_flag == true) {
      ...
      // script_h.getCurrent()[0], script_h.getCurrent()[0]);
  ...
  if (selectvoice_file_name[SELECTVOICE_SELECT])
    playSound(selectvoice_file_name[SELECTVOICE_SELECT], SOUND_CHUNK, false, MIX_WAVE_CHANNEL);
...
int ONScripter::ofscopyCommand() {
  ...
  SDL_RenderReadPixels(renderer, &render_view_rect, tmp_surface->format->format, tmp_surface->pixels,
                       tmp_surface->pitch);
...
int ONScripter::mp3stopCommand() {
  ...
  if (Mix_PlayingMusic() == 1 && mp3fadeout_duration > 0) {
    ...
    if (ext && (!strcmp(ext + 1, "OGG") || !strcmp(ext + 1, "ogg"))) {
      ...
      waitEvent(0);
      setStr(&music_file_name,
             NULL); // to ensure not to play music during fadeout
...
int ONScripter::mp3Command() {
  ...
  if (script_h.isName("mp3save")) {
    ...
    mp3save_flag = false;
  } else if (script_h.isName("mp3loop") || script_h.isName("bgm")) {
    mp3save_flag = true;
    loop_flag = true;
  } else {
    mp3save_flag = false;
  ...
  if (buf[0] != '\0') {
    ...
    playSound(music_file_name, SOUND_MUSIC | SOUND_MIDI | SOUND_CHUNK, music_play_loop_flag, MIX_BGM_CHANNEL);
    ...
    if (mp3fadein_duration > 0) {
      ...
      mp3fade_start = SDL_GetTicks();
      timer_bgmfade_id = SDL_AddTimer(20, bgmfadeCallback, (void *)&timer_bgmfade_id);
...
int ONScripter::lsp2Command() {
  ...
  bool v = true;
  if (script_h.isName("lsph2") || script_h.isName("lsph2add") || script_h.isName("lsph2sub"))
    v = false;
...
int ONScripter::loopbgmCommand() {
  ...
  playSound(loop_bgm_name[1], SOUND_PRELOAD | SOUND_CHUNK, false, MIX_LOOPBGM_CHANNEL1);
...
static void smpeg_filter_callback(SDL_Overlay *dst, SDL_Overlay *src, SDL_Rect *region, SMPEG_FilterInfo *filter_info,
                                  void *data) {
  if (dst) {
    dst->w = 0;
    dst->h = 0;
...
int ONScripter::gettagCommand() {
  ...
  do {
    ...
    if (script_h.pushed_variable.type & ScriptHandler::VAR_INT ||
      ...
      if (buf) {
        ...
        while (*buf != '/' && *buf != 0 && *buf != ']' && (!zenkakko_flag || buf[0] != "¡Û"[0] || buf[1] != "¡Û"[1])) {
          if (IS_TWO_BYTE(*buf))
            buf += 2;
          else
            buf++;
  ...
  if (pretext_buf[0] == ']')
    pretext_buf++;
  else if (zenkakko_flag && pretext_buf[0] == "¡Û"[0] && pretext_buf[1] == "¡Û"[1])
    pretext_buf += 2;
...
int ONScripter::getscreenshotCommand() {
  ...
  SDL_RenderReadPixels(renderer, &render_view_rect, screenshot_surface->format->format, screenshot_surface->pixels,
    ...
    if (reg_buf[0] == '[') {
      ...
      if (!strncmp(reg_buf + 1, path, (c - 1 > strlen(path)) ? (c - 1) : strlen(path))) {
        while (fgets(reg_buf2, 256, fp)) {
          ...
          buf = script_h.readStr();
          if (strncmp(buf, key, (strlen(buf) > strlen(key)) ? strlen(buf) : strlen(key))) {
            script_h.popCurrent();
            continue;
          }
...
int ONScripter::exbtnCommand() {
  ...
  if (script_h.isName("exbtn_d") || script_h.isName("bdef")) {
...
int ONScripter::btnwaitCommand() {
  ...
  if ((textbtn_flag || bexec_flag) &&
      (skip_mode & SKIP_NORMAL || (skip_mode & SKIP_TO_EOP && (textgosub_clickstr_state & 0x03) == CLICK_WAIT) ||
       ctrl_pressed_status)) {
    waitEventSub(0); // for checking keyup event
    current_button_state.button = 0;
    if (bexec_flag)
      current_button_state.button = -1;
    if (skip_mode & SKIP_NORMAL || (skip_mode & SKIP_TO_EOP && (textgosub_clickstr_state & 0x03) == CLICK_WAIT))
    ...
    if (textbtn_flag) {
      ...
      if (btntime_value == -1) {
        if (automode_flag) {
          ...
          // current_button_state.button = 0;
        } else if (autoclick_time > 0 && (t == -1 || t > autoclick_time))
          t = autoclick_time;
      }
    }
...
int ONScripter::btnCommand() {
  ...
  if (btndef_info.image_surface && src_rect.x + button->image_rect.w > btndef_info.image_surface->w) {
    button->image_rect.w = btndef_info.image_surface->w - src_rect.x;
  }
  if (btndef_info.image_surface && src_rect.y + button->image_rect.h > btndef_info.image_surface->h) {
...
int ONScripter::bspCommand() {
  ...
  if (sprite_info[no].image_surface || sprite_info[no].trans_mode == AnimationInfo::TRANS_STRING)
...
static SDL_Texture *createMaximumTexture(SDL_Renderer *renderer, SDL_Rect &blt_rect, const SDL_Rect &src_rect,
...
void ONScripter::NSDSp2Command(int texnum, int dcx, int dcy, int sx, int sy, int w, int h, int xs, int ys, int rot,
                               int alpha) {
  if (texnum < 0 || texnum >= MAX_TEXTURE_NUM)
    return;
