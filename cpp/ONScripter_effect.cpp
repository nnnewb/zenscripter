...
bool ONScripter::setEffect(EffectLink *effect, bool generate_effect_dst, bool update_backup_surface) {
  ...
  if (effect_no == 11 || effect_no == 12 || effect_no == 13 || effect_no == 14 || effect_no == 16 || effect_no == 17)
...
bool ONScripter::doEffect(EffectLink *effect, bool clear_dirty_region) {
  ...
  // utils::printInfo("effect_counter %d timer between %d
  ...
  case 15: // Fade with mask
    alphaBlend(effect->anim.image_surface, ALPHA_BLEND_FADE_MASK, 256 * effect_counter / effect_duration,
  ...
  case 18: // Cross fade with mask
    alphaBlend(effect->anim.image_surface, ALPHA_BLEND_CROSSFADE_MASK, 256 * effect_counter * 2 / effect_duration,
               &dirty_rect.bounding_box);
    break;
  ...
  case (MAX_EFFECT_NUM + 0): // quakey
    ...
    amp = (Sint16)(sin(M_PI * 2.0 * effect->no * effect_counter / effect_duration) * EFFECT_QUAKE_AMP * effect->no *
                   (effect_duration - effect_counter) / effect_duration);
    dst_rect.x = 0;
    dst_rect.y = amp;
    drawEffect(&dst_rect, &src_rect, effect_dst_surface);
  ...
  case (MAX_EFFECT_NUM + 3): // flushout
    if (effect_counter > 0) {
      ...
      if (height > width) {
        ...
        alphaBlend(NULL, ALPHA_BLEND_CONST, 64, &dirty_rect.bounding_box, effect_tmp_surface, accumulation_surface,
                   effect_tmp_surface);
  ...
  waitEvent(0);
  if (!((automode_flag || autoclick_time > 0) || (usewheel_flag && current_button_state.button == -5) ||
