...
void ONScripter::flushEventSub(SDL_Event &event) {
  if (event.type == ONS_MUSIC_EVENT) {
    if (music_play_loop_flag || (cd_play_loop_flag && !cdaudio_flag)) {
      stopBGM(true);
      if (music_file_name) {
  ...
  } else if ((event.type == ONS_BGMFADE_EVENT) && (event.user.code == BGM_FADEOUT)) {
    ...
    if (tmp < cur_fade_duration) {
      ...
      if (ext && (strcmp(ext + 1, "OGG") && strcmp(ext + 1, "ogg"))) {
        // set break event to return to script processing when playing music
        // other than ogg
        SDL_Event event;
  ...
  } else if ((event.type == ONS_BGMFADE_EVENT) && (event.user.code == BGM_FADEIN)) {
    ...
    if (wave_sample[event.user.code]) {
      ...
      if (event.user.code == MIX_LOOPBGM_CHANNEL0 && loop_bgm_name[1] && wave_sample[MIX_LOOPBGM_CHANNEL1])
...
void ONScripter::removeBGMFadeEvent() { removeEvent(ONS_BGMFADE_EVENT); }
...
bool ONScripter::trapHandler() {
  if (event_mode & WAIT_BUTTON_MODE || event_mode & WAIT_TEXT_MODE)
    return false;
...
bool ONScripter::mouseMoveEvent(SDL_MouseMotionEvent *event) {
  ...
  if (event_mode & WAIT_BUTTON_MODE) {
    ...
    if (getmouseover_flag && current_over_button >= getmouseover_lower && current_over_button <= getmouseover_upper) {
...
bool ONScripter::mousePressEvent(SDL_MouseButtonEvent *event) {
  ...
  if (event->button == SDL_BUTTON_RIGHT && event->type == SDL_MOUSEBUTTONUP &&
      ((rmode_flag && event_mode & WAIT_TEXT_MODE) || (event_mode & (WAIT_BUTTON_MODE | WAIT_RCLICK_MODE)))) {
  ...
  } else if (event->button == SDL_BUTTON_LEFT && (event->type == SDL_MOUSEBUTTONUP || btndown_flag)) {
    current_button_state.button = current_over_button;
    ...
    if (event->type == SDL_MOUSEBUTTONDOWN)
      current_button_state.down_flag = true;
           ...
           (bexec_flag || (event_mode & WAIT_TEXT_MODE) || (usewheel_flag && event_mode & WAIT_BUTTON_MODE) ||
    ...
    if (event_mode & WAIT_TEXT_MODE)
      system_menu_mode = SYSTEM_LOOKBACK;
             ...
             (bexec_flag || (enable_wheeldown_advance_flag && event_mode & WAIT_TEXT_MODE) ||
              (usewheel_flag && event_mode & WAIT_BUTTON_MODE) || system_menu_mode == SYSTEM_LOOKBACK)) {
    if (event_mode & WAIT_TEXT_MODE)
      current_button_state.button = 0;
    else
...
bool ONScripter::mouseWheelEvent(SDL_MouseWheelEvent *event) {
  ...
  if (event->y > 0 && (bexec_flag || (event_mode & WAIT_TEXT_MODE) ||
                       (usewheel_flag && event_mode & WAIT_BUTTON_MODE) || system_menu_mode == SYSTEM_LOOKBACK)) {
    current_button_state.button = -2;
    sprintf(current_button_state.str, "WHEELUP");
    if (event_mode & WAIT_TEXT_MODE)
...
#undef SDLK_KP_
  ...
  if (variable_edit_mode == EDIT_SELECT_MODE) {
    ...
    sprintf(wm_edit_string, "%sCurrent %s=%d  New value? %s%d", EDIT_MODE_PREFIX, var_name, p,
...
bool ONScripter::keyPressEvent(SDL_KeyboardEvent *event) {
  ...
  if ((trap_mode & TRAP_LEFT_CLICK) &&
      (event->keysym.sym == SDLK_RETURN || event->keysym.sym == SDLK_KP_ENTER || event->keysym.sym == SDLK_SPACE)) {
    if (trapHandler())
      return true;
  } else if ((trap_mode & TRAP_RIGHT_CLICK) && (event->keysym.sym == SDLK_ESCAPE)) {
    if (trapHandler())
      return true;
  }

  if (event_mode & WAIT_BUTTON_MODE &&
      (((event->type == SDL_KEYUP || btndown_flag) && ((!getenter_flag && event->keysym.sym == SDLK_RETURN) ||
  ...
  if ((event_mode & (WAIT_INPUT_MODE | WAIT_BUTTON_MODE)) && (autoclick_time == 0 || (event_mode & WAIT_BUTTON_MODE))) {
    if (!useescspc_flag && event->keysym.sym == SDLK_ESCAPE) {
    ...
    } else if (((!getcursor_flag && event->keysym.sym == SDLK_LEFT) || event->keysym.sym == SDLK_h) &&
               (event_mode & WAIT_TEXT_MODE || (usewheel_flag && !getcursor_flag && event_mode & WAIT_BUTTON_MODE) ||
                system_menu_mode == SYSTEM_LOOKBACK)) {
      current_button_state.button = -2;
      sprintf(current_button_state.str, "WHEELUP");
      if (event_mode & WAIT_TEXT_MODE)
        system_menu_mode = SYSTEM_LOOKBACK;
    } else if (((!getcursor_flag && event->keysym.sym == SDLK_RIGHT) || event->keysym.sym == SDLK_l) &&
      ...
      if (event_mode & WAIT_TEXT_MODE)
        ...
        current_button_state.button = -3;
      sprintf(current_button_state.str, "WHEELDOWN");
    } else if (((!getcursor_flag && event->keysym.sym == SDLK_UP) || event->keysym.sym == SDLK_k ||
                event->keysym.sym == SDLK_p) &&
               event_mode & WAIT_BUTTON_MODE) {
      shiftCursorOnButton(1);
      return false;
    } else if (((!getcursor_flag && event->keysym.sym == SDLK_DOWN) || event->keysym.sym == SDLK_j ||
    ...
    } else if (getfunction_flag && event->keysym.sym >= SDLK_F1 && event->keysym.sym <= SDLK_F12) {
      current_button_state.button = -21 - (event->keysym.sym - SDLK_F1);
      sprintf(current_button_state.str, "F%d", event->keysym.sym - SDLK_F1 + 1);
    } else if (bexec_flag && event->keysym.sym >= SDLK_0 && event->keysym.sym <= SDLK_9) {
      current_button_state.button = -1; // dummy
      sprintf(current_button_state.str, "%d", event->keysym.sym - SDLK_0);
    } else if (bexec_flag && event->keysym.sym >= SDLK_a && event->keysym.sym <= SDLK_z) {
      current_button_state.button = -1; // dummy
      sprintf(current_button_state.str, "%c", 'A' + event->keysym.sym - SDLK_a);
    } else if (bexec_flag && (event->keysym.sym == SDLK_RSHIFT || event->keysym.sym == SDLK_LSHIFT)) {
      current_button_state.button = -1; // dummy
  ...
  if (event_mode & WAIT_INPUT_MODE && (autoclick_time == 0 || (event_mode & WAIT_BUTTON_MODE))) {
...
void ONScripter::runEventLoop() {
  ...
  while (SDL_WaitEvent(&event)) {
...
#else
      if (SDL_PeepEvents(&tmp_event, 1, SDL_PEEKEVENT, SDL_ALLEVENTS) == 0)
        break;
    ...
    case SDL_MOUSEMOTION:
      if (mouseMoveEvent(&event.motion))
        return;
      if (btndown_flag) {
    ...
    case ONS_CHUNK_EVENT:
      ...
      // wave_sample[0], automode_flag, event_mode);
