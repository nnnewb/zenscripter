...
#include <Files.h>
extern "C" void c2pstrcpy(Str255 dst, const char *src); // #include
                                                        // <TextUtils.h>
#elif defined(PSP)
...
void ONScripter::searchSaveFile(SaveFileInfo &save_file_info, int no) {
  ...
  WCHAR file_nameW[256];
  MultiByteToWideChar(CP_ACP, 0, file_name, -1, file_nameW, 256);
  handle = CreateFile(file_nameW, GENERIC_READ, 0, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
#else
  handle = CreateFile(file_name, GENERIC_READ, 0, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
#endif
...
int ONScripter::loadSaveFile(int no) {
  ...
  for (i = 0; i < (int)strlen(SAVEFILE_MAGIC_NUMBER); i++)
    if (readChar() != SAVEFILE_MAGIC_NUMBER[i])
      break;
