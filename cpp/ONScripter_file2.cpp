...
int ONScripter::loadSaveFile2(int file_version) {
  ...
  if (num_nest > 0) {
    ...
    while (num_nest > 0) {
      ...
      if (root_nest_info.next)
        root_nest_info.next->previous = info;
  ...
  if (music_file_name) {
    playSound(music_file_name, SOUND_MUSIC | SOUND_MIDI, music_play_loop_flag, MIX_BGM_CHANNEL);
  }
...
void ONScripter::saveSaveFile2(bool output_flag) {
  ...
  writeInt(script_h.screen_width / 4,
           output_flag); // added in version 206, changed in version 208
  writeInt(script_h.screen_width / 2,
           output_flag); // added in version 206, changed in version 208
  writeInt(script_h.screen_width * 3 / 4,
           output_flag);                  // added in version 206, changed in version 208
