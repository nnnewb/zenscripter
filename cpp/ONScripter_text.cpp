...
#define IS_ROTATION_REQUIRED(x)                                                                                        \
  (!IS_TWO_BYTE(*(x)) || (*(x) == (char)0x81 && *((x) + 1) == (char)0x50) ||                                           \
...
#define IS_TRANSLATION_REQUIRED(x) (*(x) == (char)0x81 && *((x) + 1) >= 0x41 && *((x) + 1) <= 0x44)
...
void ONScripter::drawGlyph(SDL_Surface *dst_surface, FontInfo *info, SDL_Color &color, char *text, int xy[2],
...
#endif
  TTF_GlyphMetrics((TTF_Font *)info->ttf_font[0], unicode, &minx, &maxx, &miny, &maxy, &advanced);
...
#endif
  dst_rect.y -=
      (TTF_FontHeight((TTF_Font *)info->ttf_font[0]) - info->font_size_xy[1] * screen_ratio1 / screen_ratio2) / 2;
...
void ONScripter::drawChar(char *text, FontInfo *info, bool flush_flag, bool lookback_flag, SDL_Surface *surface,
  ...
  for (int i = 0; i < 2; i++) {
    ...
    if (surface == accumulation_surface && !flush_flag && (!clip || AnimationInfo::doClipping(&dst_rect, clip) == 0)) {
...
void ONScripter::drawString(const char *str, uchar3 color, FontInfo *info, bool flush_flag, SDL_Surface *surface,
                            SDL_Rect *rect, AnimationInfo *cache_info, bool pack_hankaku) {
  int i;
  ...
  for (int i = 0; i < current_page->text_count; i++) {
    if (current_page->text[i] == 0x0a) {
      ...
      if (IS_TWO_BYTE(out_text[0])) {
        ...
        if (i + 1 != current_page->text_count && current_page->text[i + 1] != 0x0a) {
          out_text[1] = current_page->text[i + 1];
          i++;
        }
      }
...
void ONScripter::leaveTextDisplayMode(bool force_leave_flag) {
  if (display_mode & DISPLAY_MODE_TEXT && (force_leave_flag || erase_text_window_mode != 0)) {
...
int ONScripter::textCommand() {
  ...
  if (buf[string_buffer_offset] == '[')
    string_buffer_offset++;
  else if (zenkakko_flag && buf[string_buffer_offset] == coding2utf16->bracket[0] &&
  ...
  while (tag_flag && buf[string_buffer_offset]) {
    if (zenkakko_flag && buf[string_buffer_offset] == coding2utf16->bracket[2] &&
  ...
  if (pretextgosub_label && (!pagetag_flag || page_enter_status == 0) && line_enter_status == 0) {
...
bool ONScripter::checkLineBreak(const char *buf, FontInfo *fi) {
  ...
  // check start kinsoku
  if (isStartKinsoku(buf + 2) || (buf[2] == '_' && isStartKinsoku(buf + 3))) {
    const char *buf2 = buf;
    if (buf2[2] == '_')
      buf2++;
    int i = 2;
...
bool ONScripter::processText() {
  ...
  // string_buffer_offset ], string_buffer_offset, event_mode,
  // line_enter_status);
  char out_text[3] = {'\0', '\0', '\0'};

  // utils::printInfo("*** textCommand %d (%d,%d)\n", string_buffer_offset,
  // sentence_font.xy[0], sentence_font.xy[1]);
  ...
  if (IS_TWO_BYTE(ch)) { // Shift jis
    ...
    return true;
  } else if (ch == '(' && (!english_mode || !(script_h.getEndStatus() & ScriptHandler::END_1BYTE_CHAR))) {
    current_page->add('(');
    ...
    return true;
             ...
             (!english_mode || !(script_h.getEndStatus() & ScriptHandler::END_1BYTE_CHAR))) {
    current_page->add('>');
    string_buffer_offset++;
    in_textbtn_flag = false;
    return true;
