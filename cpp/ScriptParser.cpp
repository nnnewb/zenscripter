...
#define VERSION_STR2                                                                                                   \
  "Copyright (C) 2001-2016 Studio O.G.A. All Rights Reserved.\n          (C) "                                         \
  "2014-2016 jh10001"
...
int ScriptParser::readInt() {
  ...
  int i = (unsigned int)file_io_buf[file_io_buf_ptr + 3] << 24 | (unsigned int)file_io_buf[file_io_buf_ptr + 2] << 16 |
          (unsigned int)file_io_buf[file_io_buf_ptr + 1] << 8 | (unsigned int)file_io_buf[file_io_buf_ptr];
  file_io_buf_ptr += 4;
...
void ScriptParser::writeStr(char *s, bool output_flag) {
  if (s && s[0]) {
    if (output_flag)
      memcpy(file_io_buf + file_io_buf_ptr, s, strlen(s));
...
int ScriptParser::readEffect(EffectLink *effect) {
  ...
  // effect->duration, effect->anim.image_name );
  return num;
}
...
bool ScriptParser::isStartKinsoku(const char *str) {
  for (int i = 0; i < num_start_kinsoku; i++) {
    if ((start_kinsoku[i].chr[0] == *str) && (start_kinsoku[i].chr[1] == *(str + 1)))
      return true;
  }
  return false;
}
...
bool ScriptParser::isEndKinsoku(const char *str) {
  for (int i = 0; i < num_end_kinsoku; i++) {
    if ((end_kinsoku[i].chr[0] == *str) && (end_kinsoku[i].chr[1] == *(str + 1)))
      return true;
  }
  return false;
}