...
int ScriptParser::setlayerCommand() {
  ...
  utils::printError("setlayer: layer effect support not available (%d,%d,'%s')", no, interval, dll);
  return RET_CONTINUE;
#else
  ...
  if ((bslash && !strncmp(bslash + 1, "oldmovie.dll", 12)) || !strncmp(dll, "oldmovie.dll", 12)) {
    handler = new OldMovieLayer(screen_width, screen_height);
  } else if ((bslash && !strncmp(bslash + 1, "snow.dll", 8)) || !strncmp(dll, "snow.dll", 8)) {
    handler = new FuruLayer(screen_width, screen_height, false, script_h.cBR);
  } else if ((bslash && !strncmp(bslash + 1, "hana.dll", 8)) || !strncmp(dll, "hana.dll", 8)) {
...
int ScriptParser::returnCommand() {
  ...
  if (textgosub_flag && (textgosub_clickstr_state & (CLICK_NEWPAGE | CLICK_EOL))) {
    line_enter_status = 0;
    page_enter_status = 0;
  }
...
int ScriptParser::nsaCommand() {
  ...
  delete script_h.cBR;
  script_h.cBR =
      new NsaReader(nsa_offset, archive_path, BaseReader::ARCHIVE_TYPE_NSA | BaseReader::ARCHIVE_TYPE_NS2, key_table);
...
int ScriptParser::ifCommand() {
  ...
  while (1) {
    if (script_h.compareString("fchk")) {
      ...
      // utils::printInfo("fchk %s(%d) ", tmp_string_buffer, (findAndAddFileLog(
      // tmp_string_buffer, fasle )) );
      ...
      if (script_h.current_variable.type & ScriptHandler::VAR_INT ||
        ...
        if ((op_buf[0] == '>' && op_buf[1] == '=') || (op_buf[0] == '<' && op_buf[1] == '=') ||
            (op_buf[0] == '=' && op_buf[1] == '=') || (op_buf[0] == '!' && op_buf[1] == '=') ||
            (op_buf[0] == '<' && op_buf[1] == '>'))
    ...
    if ((condition_status == 2 && !condition_flag) || (condition_status != 2 && !f))
