...
void rescaleImage(unsigned char *original_buffer, int width, int height, int byte_per_pixel, bool src_pad_flag,
                  bool dst_pad_flag, bool palette_flag) {
  size_t width_pad = 0;
  if (src_pad_flag)
  ...
  resizeImage(rescaled_tmp_buffer, w, h, w * byte_per_pixel + w_pad, original_buffer, width, height,
              width * byte_per_pixel + width_pad, byte_per_pixel, rescaled_tmp2_buffer,
              width * byte_per_pixel + width_pad, palette_flag);
}
...
size_t rescaleJPEGWrite(unsigned int width, unsigned int height, int byte_per_pixel, unsigned char **rescaled_buffer,
  ...
  while (cinfo2.next_scanline < cinfo2.image_height) {
    if (bmp2jpeg_flag) {
      unsigned char *src = row_pointer[0] =
          &rescaled_tmp_buffer[(cinfo2.image_height - 1 - cinfo2.next_scanline) * row_stride];
...
size_t rescaleJPEG(unsigned char *original_buffer, size_t length, unsigned char **rescaled_buffer, int quality) {
  ...
  size_t datacount = rescaleJPEGWrite(cinfo.output_width, cinfo.output_height, cinfo.output_components, rescaled_buffer,
                                      quality, false);
...
void rescaleBMPWrite(unsigned char *original_buffer, size_t total_size, int width, int height,
...
size_t rescaleBMP(unsigned char *original_buffer, unsigned char **rescaled_buffer, bool output_jpeg_flag, int quality) {
  ...
  int color_num =
      original_buffer[46] + ((int)original_buffer[47] << 8) + (original_buffer[48] << 16) + (original_buffer[49] << 24);
