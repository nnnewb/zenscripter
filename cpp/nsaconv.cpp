...
extern size_t rescaleJPEG(unsigned char *original_buffer, size_t length, unsigned char **rescaled_buffer, int quality);
extern size_t rescaleBMP(unsigned char *original_buffer, unsigned char **rescaled_buffer, bool output_jpeg_flag,
                         int quality);
...
void help() {
  fprintf(stderr, "Usage: nsaconv [-e] [-j] [-ns2] [-ns3] [-q quality] "
                  "src_width dst_width src_archive_file dst_archive_file\n");
...
int main(int argc, char **argv) {
  ...
  for (i = 0; i < count; i++) {
    ...
    if ((strlen(sFI.name) > 3 && !strcmp(sFI.name + strlen(sFI.name) - 3, "JPG")) ||
      ...
      cSR.putFile(fp, i, sFI.offset, sFI.length, sFI.length,
                  enhanced_flag ? BaseReader::NBZ_COMPRESSION : sFI.compression_type, true, rescaled_buffer);
