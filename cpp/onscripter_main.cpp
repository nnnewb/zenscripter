...
void optionHelp() {
  ...
  printf("      --enable-wheeldown-advance\tadvance the text on mouse wheel "
         "down\n");
  printf("      --disable-rescale\tdo not rescale the images in the archives\n");
  printf("      --render-font-outline\trender the outline of a text instead of "
         "casting a shadow\n");
  printf("      --edit\t\tenable online modification of the volume and "
  ...
  printf("  -SW, -HW\t\tuse software or hardware renderer (software renderer "
         "is used by default)\n");
...
JNIEXPORT jint JNICALL JAVA_EXPORT_NAME(ONScripter_nativeGetWidth)(JNIEnv *env, jobject thiz) { return ons.getWidth(); }
...
#if defined(ZIP_URL)
  if ([[[DataDownloader alloc] init] download])
    exit(-1);
#endif
