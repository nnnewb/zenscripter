...
AnimationInfo::AnimationInfo(const AnimationInfo &anim) { *this = anim; }
...
bool AnimationInfo::proceedAnimation(int current_time) {
  if (!visible || !is_animatable || next_time > current_time)
    return false;
...
int AnimationInfo::doClipping(SDL_Rect *dst, SDL_Rect *clip, SDL_Rect *clipped) {
  ...
  if (!dst || dst->x >= clip->x + clip->w || dst->x + dst->w <= clip->x || dst->y >= clip->y + clip->h ||
      dst->y + dst->h <= clip->y)
    return -1;
...
#define SUBBLEND_PIXEL()                                                                                               \
  {                                                                                                                    \
    Uint32 mask2 = (*alphap * alpha) >> 8;                                                                             \
    Uint32 mask_r = (*dst_buffer & RMASK) - ((((*src_buffer & RMASK) * mask2) >> 8) & RMASK);                          \
    mask_r &= ((mask_r & AMASK) ? 0 : RMASK);                                                                          \
    Uint32 mask_g = (*dst_buffer & GMASK) - ((((*src_buffer & GMASK) * mask2) >> 8) & GMASK);                          \
    mask_g &= ((mask_g & ~(GMASK | BMASK)) ? 0 : GMASK);                                                               \
    Uint32 mask_b = (*dst_buffer & BMASK) - ((((*src_buffer & BMASK) * mask2) >> 8) & BMASK);                          \
...
void AnimationInfo::blendOnSurface(SDL_Surface *dst_surface, int dst_x, int dst_y, SDL_Rect &clip, int alpha) {
  ...
  dst_rect.w = pos.w;
  dst_rect.h = pos.h;
  if (doClipping(&dst_rect, &clip, &src_rect))
    return;
  if (alpha == 0)
    return;
  ...
  } blender = {(ONSBuf *)image_surface->pixels + pitch * src_rect.y + image_surface->w * current_cell / num_of_cells +
                   src_rect.x,
...
void AnimationInfo::blendOnSurface2(SDL_Surface *dst_surface, int dst_x, int dst_y, SDL_Rect &clip, int alpha) {
  ...
  int max_xy[2] = {bounding_rect.x + bounding_rect.w - 1, bounding_rect.y + bounding_rect.h - 1};

  // clip bounding box
  if (max_xy[0] < clip.x)
    return;
  if (max_xy[0] >= clip.x + clip.w)
    max_xy[0] = clip.x + clip.w - 1;
  if (min_xy[0] >= clip.x + clip.w)
    return;
  if (min_xy[0] < clip.x)
    min_xy[0] = clip.x;
  if (max_xy[1] < clip.y)
    return;
  if (max_xy[1] >= clip.y + clip.h)
    max_xy[1] = clip.y + clip.h - 1;
  if (min_xy[1] >= clip.y + clip.h)
    return;
  if (min_xy[1] < clip.y)
    min_xy[1] = clip.y;
  ...
  if (min_xy[1] < 0)
    min_xy[1] = 0;
  if (max_xy[1] >= dst_surface->h)
    max_xy[1] = dst_surface->h - 1;
  ...
  if (src_rect[0][1] < 0)
    src_rect[0][1] = 0;
  if (src_rect[1][0] >= pos.w)
    src_rect[1][0] = pos.w - 1;
  if (src_rect[1][1] >= pos.h)
    src_rect[1][1] = pos.h - 1;
  ...
  struct Blender {
    ...
    void operator()(const int y) const {
      ...
      for (int i = 0; i < 4; i++) {
        int i2 = (i + 1) & 3; // = (i+1)%4
        if (corner_xy[i][1] == corner_xy[i2][1])
          continue;
        int x = (corner_xy[i2][0] - corner_xy[i][0]) * (y - corner_xy[i][1]) / (corner_xy[i2][1] - corner_xy[i][1]) +
                corner_xy[i][0];
      ...
      if (raster_min < 0)
        raster_min = 0;
      if (raster_max >= dst_surface->w)
        raster_max = dst_surface->w - 1;
      ...
      for (int x = raster_min - dst_x; x <= raster_max - dst_x; x++, dst_buffer++) {
        ...
        if (x2 < src_rect[0][0] || x2 >= src_rect[1][0] || y2 < src_rect[0][1] || y2 >= src_rect[1][1])
          continue;
  ...
  {                                                                                                                    \
    ...
    if (mask2 == 255) {                                                                                                \
      *dst_buffer = src_color;                                                                                         \
      ...
      Uint32 mask_rb = ((*dst_buffer & 0xff00ff) * mask1 + src_color1 * mask2);                                        \
      mask_rb = (((mask_rb >> 16) * alpha) & 0x00ff0000) | (((mask_rb & 0xffff) * alpha >> 16) & 0xff);                \
      Uint32 mask_g = (((*dst_buffer & 0x00ff00) * mask1 + src_color2 * mask2) * alpha >> 16) & 0x00ff00;              \
...
void AnimationInfo::blendText(SDL_Surface *surface, int dst_x, int dst_y, SDL_Color &color, SDL_Rect *clip,
                              bool rotate_flag) {
  if (image_surface == NULL || surface == NULL)
    return;
  ...
  if (doClipping(&dst_rect, &clip_rect, &clipped_rect))
    return;
  ...
  int pitch = image_surface->pitch / sizeof(ONSBuf);
  Uint32 src_color1 = (((color.r >> fmt->Rloss) << fmt->Rshift) | ((color.b >> fmt->Bloss) << fmt->Bshift));
  Uint32 src_color2 = ((color.g >> fmt->Gloss) << fmt->Gshift);
  Uint32 src_color = src_color1 | src_color2 | fmt->Amask;

  ONSBuf *dst_buffer = (ONSBuf *)image_surface->pixels + pitch * dst_rect.y +
                       image_surface->w * current_cell / num_of_cells + dst_rect.x;
  ...
  if (!rotate_flag) {
    ...
    for (int i = 0; i < dst_rect.h; i++) {
      unsigned char *src_buffer =
          (unsigned char *)surface->pixels + surface->pitch * (surface->h - src_rect.x - 1) + src_rect.y + i;
      for (int j = dst_rect.w; j != 0; j--) {
        BLEND_TEXT_ALPHA();
...
void AnimationInfo::fill(Uint8 r, Uint8 g, Uint8 b, Uint8 a) {
  ...
  SDL_PixelFormat *fmt = image_surface->format;
  Uint32 rgb = (((r >> fmt->Rloss) << fmt->Rshift) | ((g >> fmt->Gloss) << fmt->Gshift) |
                ((b >> fmt->Bloss) << fmt->Bshift) | ((a >> fmt->Aloss) << fmt->Ashift));
...
SDL_Surface *AnimationInfo::setupImageAlpha(SDL_Surface *surface, SDL_Surface *surface_m, bool has_alpha) {
  ...
  if (trans_mode == TRANS_TOPLEFT) {
    ...
    ref_color = direct_color[0] << fmt->Rshift | direct_color[1] << fmt->Gshift | direct_color[2] << fmt->Bshift;
  }
  ref_color &= 0xffffff;
  ...
  if (trans_mode == TRANS_ALPHA && !has_alpha) {
    ...
    SDL_Surface *surface2 =
        SDL_CreateRGBSurface(SDL_SWSURFACE, w3, h, fmt->BitsPerPixel, fmt->Rmask, fmt->Gmask, fmt->Bmask, fmt->Amask);
  ...
  } else if (trans_mode == TRANS_TOPLEFT || trans_mode == TRANS_TOPRIGHT || trans_mode == TRANS_DIRECT) {
    for (i = h; i != 0; i--) {
...
void AnimationInfo::convertFromYUV(SDL_Overlay *src) {
  ...
  for (int i = 0; i < ls->h; i++) {
    ...
    for (int j = 0; j < ls->w; j++) {
      ...
      *(p + j) = (((r >> (10 + fmt->Rloss)) & 0xff) << fmt->Rshift) |
                 (((g >> (10 + fmt->Gloss)) & 0xff) << fmt->Gshift) |
                 (((b >> (10 + fmt->Bloss)) & 0xff) << fmt->Bshift) | AMASK;
    }
  }
