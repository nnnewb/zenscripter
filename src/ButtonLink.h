...
struct ButtonLink {
  enum { NORMAL_BUTTON = 0, SPRITE_BUTTON = 1, LOOKBACK_BUTTON = 2, TMP_SPRITE_BUTTON = 3 };
  ...
  ~ButtonLink() {
    if ((button_type == NORMAL_BUTTON || button_type == TMP_SPRITE_BUTTON) && anim[0])
      delete anim[0];
    for (int i = 0; i < 3; i++)
      if (exbtn_ctl[i])
        delete[] exbtn_ctl[i];
  ...
  void removeSprite(int no) {
    ButtonLink *bl = this;
    while (bl->next) {
      if (bl->next->sprite_no == no && bl->next->button_type == SPRITE_BUTTON) {
