...
void *FontInfo::openFont(char *font_file, int ratio1, int ratio2) {
  ...
  if (!fc->next) {
    ...
    if (useFile)
      ...
      fc->next->font[0] =
          TTF_OpenFontRW(SDL_RWFromConstMem(font_cache, font_cache_size), 1, font_size * ratio1 / ratio2);
#if (SDL_TTF_MAJOR_VERSION >= 2) && (SDL_TTF_MINOR_VERSION >= 0) && (SDL_TTF_PATCHLEVEL >= 10)
    if (useFile)
...
int FontInfo::getTateyokoMode() { return tateyoko_mode; }
...
void FontInfo::advanceCharInHankaku(int offset) { xy[tateyoko_mode] += offset; }

void FontInfo::addLineOffset(int offset) { line_offset_xy[tateyoko_mode] += offset; }
...
int FontInfo::initRuby(FontInfo &body_info, int body_count, int ruby_count) {
  if ((tateyoko_mode == YOKO_MODE && body_count + body_info.xy[0] / 2 >= body_info.num_xy[0] - 1) ||
      (tateyoko_mode == TATE_MODE && body_count + body_info.xy[1] / 2 > body_info.num_xy[1]))
    body_info.newLine();
  ...
  if (ruby_count * font_size_xy[tateyoko_mode] >= body_count * body_info.pitch_xy[tateyoko_mode]) {
    ...
    if (ruby_count > 0)
      offset =
          (body_count * body_info.pitch_xy[tateyoko_mode] - ruby_count * font_size_xy[tateyoko_mode] + ruby_count) /
          (ruby_count * 2);
