...
int NL_dofile(lua_State *state) {
  ...
  if (luaL_loadbuffer(state, (const char *)buffer2, p2 - buffer2, str) || lua_pcall(state, 0, 0, 0)) {
...
int NSDDLL(lua_State *state) {
  ...
  while (dll_func_table[idx].name1) {
    ...
    if (strlen(str1) >= len && strncmp(str1 + strlen(str1) - len, dll_func_table[idx].name1, len) == 0 &&
...
int NSDSp2(lua_State *state) {
  ...
  lh->ons->NSDSp2Command(num, dcx, dcy, sx, sy, w, h, (int)(xs * 100.0), (int)(xy * 100.0), (int)rot, alpha);
...
int NSGetKey(lua_State *state) {
  ...
  if (strcmp(str, bs.str) == 0 || (strcmp(str, "ESC") == 0 && strcmp(bs.str, "RCLICK") == 0))
    lua_pushboolean(state, 1);
  else
    lua_pushboolean(state, 0);
...
int lua_dummy(lua_State *state) { return 0; }
...
static const struct luaL_Reg lua_lut[] = {LUA_FUNC_LUT(NL_dofile),
...
static const struct luaL_Reg module_dpshadow[] = {
    LUA_FUNC_LUT_DUMMY(dpshadow_make), LUA_FUNC_LUT_DUMMY(dpshadow_merge), {NULL, NULL}};
...
void LUAHandler::init(ONScripter *ons, ScriptHandler *sh, int screen_ratio1, int screen_ratio2) {
  this->ons = ons;
  this->sh = sh;
...
void LUAHandler::loadInitScript() {
  ...
  if (luaL_loadbuffer(state, (const char *)buffer2, p2 - buffer2, INIT_SCRIPT) || lua_pcall(state, 0, 0, 0)) {
...
bool LUAHandler::isCallbackEnabled(int val) { return callback_state[val]; }
