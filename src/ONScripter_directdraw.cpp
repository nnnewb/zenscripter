...
void DirectDraw::draw2(int no, int dcx, int dcy, int sx, int sy, int w, int h, float xs, float ys, float rot, int alpha,
                       bool add) {
  assert(no >= 0 && no < MAX_TEXTURE_NUM);
  if (!texture_info[no])
    return;
...
void DirectDraw::present() { SDL_RenderPresent(ons.renderer); }
