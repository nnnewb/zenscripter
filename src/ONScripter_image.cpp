...
SDL_Surface *ONScripter::loadImage(char *filename, bool *has_alpha, int *location, unsigned char *alpha) {
  ...
  if ((tmp->w * tmp->format->BytesPerPixel == tmp->pitch) &&
      ...
      (tmp->format->Rmask == image_surface->format->Rmask) && (tmp->format->Gmask == image_surface->format->Gmask) &&
      (tmp->format->Bmask == image_surface->format->Bmask) && (tmp->format->Amask == image_surface->format->Amask)) {
    ret = tmp;
  } else {
...
SDL_Surface *ONScripter::createRectangleSurface(char *filename, bool *has_alpha, unsigned char *alpha) {
  ...
  SDL_Surface *tmp =
      SDL_CreateRGBSurface(SDL_SWSURFACE, w, h, fmt->BitsPerPixel, fmt->Rmask, fmt->Gmask, fmt->Bmask, fmt->Amask);
...
int ONScripter::resizeSurface(SDL_Surface *src, SDL_Surface *dst) {
  ...
  resizeImage((unsigned char *)dst_buffer, dst->w, dst->h, dst->w * 4, (unsigned char *)src_buffer, src->w, src->h,
              src->w * 4, 4, resize_buffer, src->w * 4, false);
...
#ifdef USE_SIMD
inline static void alphaBlendCore32(Uint32 *src1_buffer, Uint32 *src2_buffer, Uint32 *dst_buffer, simd::uint16x8 m_lo,
...
inline static void alphaBlendPixelCore32(Uint32 *src1_buffer, Uint32 *src2_buffer, Uint32 *dst_buffer, Uint8 mask,
                                         simd::ivec128 zero) {
  using namespace simd;
...
inline static void alphaBlend32(Uint32 *src1_buffer, Uint32 *src2_buffer, Uint32 *dst_buffer, const Uint32 *mask_buffer,
                                Uint32 mask_value, Uint32 overflow_mask, Uint32 mask_surface_w, int rect_x,
                                int rect_w) {
  int j2 = rect_x;
#ifdef USE_SIMD
  using namespace simd;
...
inline static void alphaBlendConst32(Uint32 *src1_buffer, Uint32 *src2_buffer, Uint32 *dst_buffer, Uint32 mask2,
                                     int remain) {
#ifdef USE_SIMD
  using namespace simd;
  ivec128 zero = ivec128::zero();
...
void ONScripter::alphaBlend(SDL_Surface *mask_surface, int trans_mode, Uint32 mask_value, SDL_Rect *clip,
    ...
    struct Blender {
      ...
      void operator()(const int i) const {
        ...
        const ONSBuf *mask_buffer =
            (ONSBuf *)mask_surface->pixels + mask_surface->w * ((rect->y + i) % mask_surface->h);
        alphaBlend32(src1_buffer, src2_buffer, dst_buffer, mask_buffer, mask_value, overflow_mask, mask_surface->w,
                     rect->x, rect->w);
      }
  ...
  {                                                                                                                    \
    ...
    if (mask2 == 255) {                                                                                                \
      *dst_buffer = src_color3;                                                                                        \
      ...
      Uint32 mask_rb = (((*dst_buffer & 0xff00ff) * mask1 + src_color1 * mask2) >> 8) & 0xff00ff;                      \
      Uint32 mask_g = (((*dst_buffer & 0x00ff00) * mask1 + src_color2 * mask2) >> 8) & 0x00ff00;                       \
...
void ONScripter::alphaBlendText(SDL_Surface *dst_surface, SDL_Rect dst_rect, SDL_Surface *src_surface, SDL_Color &color,
  ...
  if (fmt->BitsPerPixel == 16) {
    Uint32 src_color = (((color.r >> fmt->Rloss) << fmt->Rshift) | ((color.g >> fmt->Gloss) << fmt->Gshift) |
    ...
    if (!rotate_flag) {
      ...
      for (int i = 0; i < dst_rect.h; i++) {
        unsigned char *src_buffer =
            (unsigned char *)src_surface->pixels + src_surface->pitch * (src_surface->h - x2 - 1) + y2 + i;
        for (int j = dst_rect.w; j != 0; j--) {
          BLEND_PIXEL_TEXT_BPP16();
...
void ONScripter::refreshSprite(int sprite_no, bool active_flag, int cell_no, SDL_Rect *check_src_rect,
...
void ONScripter::createBackground() {
  ...
  } else if (!strcmp(bg_info.file_name, "black") || !strcmp(bg_info.file_name, "*bgcpy")) {
