...
void ONScripter::executeSystemMenu() {
  ...
  if (menuselectvoice_file_name[MENUSELECTVOICE_OPEN])
    playSound(menuselectvoice_file_name[MENUSELECTVOICE_OPEN], SOUND_CHUNK, false, MIX_WAVE_CHANNEL);
  ...
  menu_font.top_xy[0] =
      (screen_width * screen_ratio2 / screen_ratio1 - menu_font.num_xy[0] * menu_font.pitch_xy[0]) / 2;
  menu_font.top_xy[1] =
      (screen_height * screen_ratio2 / screen_ratio1 - menu_font.num_xy[1] * menu_font.pitch_xy[1]) / 2;
  menu_font.setXY((menu_font.num_xy[0] - rmenu_link_width) / 2, (menu_font.num_xy[1] - rmenu_link_num) / 2);
  ...
  if (current_button_state.button == -1) {
    if (menuselectvoice_file_name[MENUSELECTVOICE_CANCEL])
      playSound(menuselectvoice_file_name[MENUSELECTVOICE_CANCEL], SOUND_CHUNK, false, MIX_WAVE_CHANNEL);
    leaveSystemCall();
    return;
  }

  if (menuselectvoice_file_name[MENUSELECTVOICE_CLICK])
    playSound(menuselectvoice_file_name[MENUSELECTVOICE_CLICK], SOUND_CHUNK, false, MIX_WAVE_CHANNEL);
...
bool ONScripter::executeSystemLoad() {
  ...
  for (unsigned int i = 1; i <= num_save_file; i++) {
    ...
    if (save_file_info.valid) {
      sprintf(buffer, MESSAGE_SAVE_EXIST, save_item_name, save_file_info.sjis_no, save_file_info.sjis_month,
              save_file_info.sjis_day, save_file_info.sjis_hour, save_file_info.sjis_minute);
      nofile_flag = false;
    } else {
      sprintf(buffer, MESSAGE_SAVE_EMPTY, save_item_name, save_file_info.sjis_no);
      nofile_flag = true;
    }
...
bool ONScripter::executeSystemYesNo(int caller, int file_no) {
  ...
  if (caller == SYSTEM_SAVE) {
    SaveFileInfo save_file_info;
    searchSaveFile(save_file_info, file_no);
    sprintf(name, MESSAGE_SAVE_CONFIRM, save_item_name, save_file_info.sjis_no);
  } else if (caller == SYSTEM_LOAD) {
    SaveFileInfo save_file_info;
    searchSaveFile(save_file_info, file_no);
    sprintf(name, MESSAGE_LOAD_CONFIRM, save_item_name, save_file_info.sjis_no);
  ...
  if (current_button_state.button == 1) { // yes is selected
    if (menuselectvoice_file_name[MENUSELECTVOICE_YES])
      playSound(menuselectvoice_file_name[MENUSELECTVOICE_YES], SOUND_CHUNK, false, MIX_WAVE_CHANNEL);
    return true;
  } else {
    if (menuselectvoice_file_name[MENUSELECTVOICE_NO])
      playSound(menuselectvoice_file_name[MENUSELECTVOICE_NO], SOUND_CHUNK, false, MIX_WAVE_CHANNEL);
    return false;
  }
}
...
void ONScripter::setupLookbackButton() {
  ...
  if ((current_page->previous->text_count > 0) && current_page != start_page) {
...
void ONScripter::executeSystemLookback() {
  ...
  while (1) {
    ...
    for (i = 0; i < 3; i++)
      sentence_font.color[i] = color[i];
    flush(REFRESH_NONE_MODE);

    event_mode = WAIT_BUTTON_MODE;
    waitEventSub(-1);

    if (current_button_state.button == 0 || (current_page == start_page && current_button_state.button == -2)) {
      continue;
    }
    if (current_button_state.button == -1 || (current_button_state.button == -3 && current_page->next == cached_page) ||
...
void ONScripter::buildDialog(bool yesno_flag, const char *mes1, const char *mes2) {
  ...
  SDL_Surface *s = SDL_CreateRGBSurface(SDL_SWSURFACE, DIALOG_W, DIALOG_H, fmt->BitsPerPixel, fmt->Rmask, fmt->Gmask,
                                        fmt->Bmask, fmt->Amask);
  ...
  if (screen_ratio2 != screen_ratio1) {
    s2 = SDL_CreateRGBSurface(SDL_SWSURFACE, DIALOG_W * screen_ratio1 / screen_ratio2,
                              DIALOG_H * screen_ratio1 / screen_ratio2, fmt->BitsPerPixel, fmt->Rmask, fmt->Gmask,
                              fmt->Bmask, fmt->Amask);
    resizeSurface(s, s2);
    SDL_FreeSurface(s);
  }
  ...
  for (int i = 0; i < 2; i++) {
    SDL_Surface *bs = SDL_CreateRGBSurface(SDL_SWSURFACE, DIALOG_BUTTON_W * 2, DIALOG_BUTTON_H, fmt->BitsPerPixel,
    ...
    SDL_Surface *bs2 = bs;
    if (screen_ratio2 != screen_ratio1) {
      bs2 = SDL_CreateRGBSurface(SDL_SWSURFACE, DIALOG_BUTTON_W * 2 * screen_ratio1 / screen_ratio2,
                                 DIALOG_BUTTON_H * screen_ratio1 / screen_ratio2, fmt->BitsPerPixel, fmt->Rmask,
    ...
    btn->anim[0]->pos.x =
        dialog_info.pos.x + (DIALOG_W - 3 - (DIALOG_BUTTON_W + 8) * (2 - i)) * screen_ratio1 / screen_ratio2;
    btn->anim[0]->pos.y =
        dialog_info.pos.y + (DIALOG_H - 3 - (DIALOG_FOOTER + DIALOG_BUTTON_H) / 2) * screen_ratio1 / screen_ratio2;
