...
void ScriptHandler::reset() {
  ...
  if (extended_variable_data)
    delete[] extended_variable_data;
  extended_variable_data = NULL;
  num_extended_variable_data = 0;
  max_extended_variable_data = 1;
...
readTokenTop:
  ...
  if (ch == ';') { // comment
  ...
  } else if (ch & 0x80 || (ch >= '0' && ch <= '9') || ch == '@' || ch == '\\' || ch == '/' || ch == '%' || ch == '?' ||
             ch == '$' || ch == '[' || ch == '(' || ch == '<' ||
#ifndef ENABLE_1BYTE_CHAR
             ch == '`' ||
#endif
             (!english_mode && ch == '>') || ch == '!' || ch == '#' || ch == ',' || ch == '"') { // text
    ...
    while (1) {
      if (IS_TWO_BYTE(ch)) {
        ...
        addStringBuffer(ch);
        buf++;
        if (!wait_script && !ignore_clickstr_flag && checkClickstr(buf - 2) > 0)
          wait_script = buf;
...
void ScriptHandler::skipToken() {
  ...
  while (1) {
    if (*buf == 0x0a || *buf == 0 || (!quat_flag && !text_flag && (*buf == ':' || *buf == ';')))
      break;
    if (*buf == '"')
      quat_flag = !quat_flag;
    if (IS_TWO_BYTE(*buf)) {
...
void ScriptHandler::setCurrent(char *pos) { current_script = next_script = pos; }
...
void ScriptHandler::setKidokuskip(bool kidokuskip_flag) { this->kidokuskip_flag = kidokuskip_flag; }
...
void ScriptHandler::enableTextgosub(bool val) { textgosub_flag = val; }
...
ScriptHandler::LogLink *ScriptHandler::findAndAddLog(LogInfo &info, const char *name, bool add_flag) {
  ...
  while (cur) {
    if (!strcmp(cur->name, capital_name))
      break;
    cur = cur->next;
  }
  if (!add_flag || cur)
...
ScriptHandler::ArrayVariable *ScriptHandler::getRootArrayVariable() { return root_array_variable; }
  ...
  if (**buf == '(') {
    ...
    while (1) {
      ...
      if ((ch >= 'a' && ch <= 'z') || (ch >= 'A' && ch <= 'Z') || ch == '_') {
        ...
        if (first_flag)
          errorAndExit("parseStr: number is not allowed for the first letter of str alias.");
        first_flag = false;
        alias_buf[alias_buf_len++] = ch;
      } else
        break;
