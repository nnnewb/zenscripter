...
static void calcWeightedSumColumnInit(unsigned char **src, int interpolation_height, int image_width, int image_height,
...
static void calcWeightedSumColumn(unsigned char **src, int y, int interpolation_height, int image_width,
                                  int image_height, int image_pixel_width, int byte_per_pixel) {
...
static void calcWeightedSum(unsigned char **dst, unsigned char **src, int x, int interpolation_width, int image_width,
                            int byte_per_pixel) {
...
void resizeImage(unsigned char *dst_buffer, int dst_width, int dst_height, int dst_total_width,
                 unsigned char *src_buffer, int src_width, int src_height, int src_total_width, int byte_per_pixel,
                 unsigned char *tmp_buffer, int tmp_total_width, bool palette_flag) {
  ...
  /* smoothing */
  if (byte_per_pixel >= 3) {
    calcWeightedSumColumnInit(&src_buf, interpolation_height, src_width, src_height, src_total_width, byte_per_pixel);
    for (i = 0; i < src_height; i++) {
      calcWeightedSumColumn(&src_buf, i, interpolation_height, src_width, src_height, src_total_width, byte_per_pixel);
      ...
      for (j = 0; j < src_width; j++)
        calcWeightedSum(&tmp_buf, &src_buf, j, interpolation_width, src_width, byte_per_pixel);
      tmp_buf += tmp_offset;
    }
  } else {
